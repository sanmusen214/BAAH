...
def set_notification(config):
    ...
    with ui.card():
        ...
        ui.input(config.get_text("email_account")).bind_value(config.softwareconfigdict, "MAIL_USER", forward = lambda x:x.replace("@","")).style("width: 300px")
        ui.input(config.get_text("email_pwd")).bind_value(config.softwareconfigdict, "MAIL_PASS").style("width: 300px")
